...
export function SplatPainting({ src, paintingKey, opacity = 1, className, style }: SplatPaintingProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // The InstancedMesh is then cover-scaled to fill the frustum.
      ...
      for (let j = 0; j < GH; j++) {
        for (let i = 0; i < GW; i++) {
          const u = (i + 0.5) / GW;
          const v = (j + 0.5) / GH;
          // Both grids are read via getImageData (top-down, row 0 = top),
          // so colour and depth share the same row index — no flip. (The
          // relief shader's `1.0 - uv.y` is a Three texture-UV convention,
          // which does not apply to direct pixel reads.)
          const idx = (j * GW + i) * 4;
          const d = depData[idx] / 255;
          const ci = idx;
          ...
          // Deterministic scatter vector for the assemble animation.
          const h = Math.sin((i * 12.9898 + j * 78.233) * 43758.5453);
          const a = (h - Math.floor(h)) * Math.PI * 2;
          const h2 = Math.sin((i * 39.346 + j * 11.135) * 12543.21);
          const r = 0.5 + (h2 - Math.floor(h2)) * 1.4;
          seeds[n * 3]     = Math.cos(a) * r;
          seeds[n * 3 + 1] = Math.sin(a) * r;
          seeds[n * 3 + 2] = (Math.sin(i * 7.13 + j * 3.7) ) * 0.6;
          n++;
        }
      }
      ...
      const draw = () => {
        ...
        // Gentle auto-orbit + cursor parallax. Small angles keep the
        // disocclusion holes (gaps behind near pigment) from showing.
        const az = Math.sin(t * 0.18) * 0.085 + pointerX * 0.05;
        const el = Math.cos(t * 0.14) * 0.04 - pointerY * 0.035;
        camera.position.set(
